...
export const CardComponent: React.FC<CardProps> = ({
  ...
  const [idFavorite, setIdFavorite] = useState<number | null | undefined>(null);
  const [user, setUser] = useState<string | null | undefined>(null);
  const [createFavorite] = useCreateFavoriteMutation();
  const [deleteFavorite] = useDeletFavoriteByIDMutation();

  const [favorito, setFavorito] = useState({
    propertyId: 0 as number | undefined,
  ...
  const handleChangeFavorite = () => {
    if (isFavorito) {
      setIsFavorito(false);
      deleteFavorite(Number(idFavorite));
  ...
  return (
    <Card key={id}>
      <CardActionArea>
        ...
        <CardContent>
          {user ? (
            <IconButton onClick={() => handleChangeFavorite()} color={isFavorito ? "primary" : "default"}>
